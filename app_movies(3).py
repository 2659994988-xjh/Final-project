...
def load_data():
    ...
    df["genre_filtered"] = df["genre"].where(
        df["genre"].isin(top_genres), "Other"
    )
...
with col1:
    ...
    box_data = [
        filtered_df[filtered_df["genre_filtered"] == g]["score"].dropna()
        for g in genres
    ]
...
with col2:
    ...
    for genre, color in zip(
        genres, plt.cm.Set2(np.linspace(0, 1, len(genres)))
    ):
    ...
    high_perf = filtered_df[
        (filtered_df["score"] >= 8.0) & (filtered_df["gross"] >= 300)
    ]
...
ax1.set_title(
    "Yearly Average Budget vs Gross (1980-1989)", fontsize=14, pad=20
)
st.pyplot(fig)

st.subheader("Movie Detail View")
selected_movie = st.selectbox(
    "Select a Movie to View Details", filtered_df["name"].tolist()
)
...
if selected_movie:
    ...
    st.write(
        f"**Year**: {year_str} | **Genre**: {genre} | **Score**: {score}"
    )
    st.write(f"**Director**: {director} | **Star**: {star}")
    st.write(
        f"**Budget**: {budget_str} Million USD | **Gross**: {gross_str} Million USD"
    )
